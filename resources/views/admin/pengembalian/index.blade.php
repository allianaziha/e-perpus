                            ...
                            <tbody>
                                @foreach ($pengembalians as $index => $data)
                                    <tr>
                                        ...
                                        <td class="text-center">Rp {{ number_format($data->denda->nominal, 0, ',', '.') }}</td>
                                        <td class="text-end">
                                            <a href="{{ route('admin.pengembalian.show', $data->id) }}" class="btn btn-sm btn-info" title="Detail"><i class="ti ti-eye"></i></a>
                                            <a href="{{ route('admin.pengembalian.edit', $data->id) }}" class="btn btn-sm btn-warning" title="Edit"><i class="ti ti-pencil"></i></a>
                                            <form action="{{ route('admin.pengembalian.destroy', $data->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
